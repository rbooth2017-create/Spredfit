...
export function ActiveWorkout({ sport, onComplete, onCancel }: ActiveWorkoutProps) {
  ...
  // 🔋 Android wake lock (keep screen on during GPS workouts)
        ...
        (position) => {
          ...
          if (lastPositionRef.current) {
            ...
            if (isAccuracyGood) {
              ...
              const timeDiff =
                (newPos.timestamp - lastPositionRef.current.timestamp) / 1000; // seconds
              ...
              const isReasonableSpeed =
                timeDiff > 0 && distanceIncrement / timeDiff < 27.78; // < 100 km/h in m/s
  ...
  // 🔋 Android screen wake lock to keep GPS running while workout is active
  useEffect(() => {
    ...
    // Only try on Android devices that support the API
    if (!isAndroid || !supportsWakeLock) {
      return;
    }
    ...
    const manageWakeLock = async () => {
      try {
        // Keep the screen awake only while a GPS-based workout is running
        if (isRunning && isDistanceTracking && gpsEnabled && gpsPermission === "granted") {
          if (!wakeLockRef.current) {
            // @ts-ignore - Wake Lock API is not yet in TypeScript's libdom
    ...
    <div className="relative min-h-screen bg-[#86a088] text-[#2d332d] pb-24">
      ...
      <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-20">
        <div
          ...
          style={{ top: "-20vh", transform: "scale(2.5)" }}
        >
          <svg
          ...
          <div className="w-full max-w-md px-6">
            ...
            <Card className="bg-[#9ca895] border-[#2d332d]/20 p-6 shadow-2xl rounded-3xl">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <p className="text-3xl text-[#2d332d] mb-1">
                ...
                {isDistanceTracking && (
                  <div>
                    <p className="text-3xl text-[#7a8872] mb-1">
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${
                      isRunning ? "bg-[#7a8872] animate-pulse" : "bg-[#2d332d]/40"
                    }`}
                  />
                ...
                {isLocked ? (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex items-center justify-end pr-12 gap-0.5 pointer-events-none opacity-40">
                      <ChevronRight
                        className="w-3 h-3 text-[#2d332d] animate-pulse"
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <span
                        className="text-xs text-[#2d332d]/60"
                        style={{
                          opacity: slidePosition > 50 ? 0 : 1,
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div
              className={`flex items-center gap-2 px-4 py-2 rounded-full ${
                isRunning
                  ? "bg-[#2d332d] border border-[#2d332d]"
                  : "bg-[#2d332d]/40 border border-[#2d332d]/40"
              }`}
            >
              <div
                className={`w-2 h-2 rounded-full ${
                  isRunning ? "bg-[#eef0ed] animate-pulse" : "bg-[#eef0ed]/60"
                }`}
              />
              <p className="text-xs text-[#eef0ed]">
                {isRunning ? "Recording" : "Paused"}
              </p>
            </div>
          </div>

          {/* GPS Status Banner + explanation */}
          {isDistanceTracking && (
            <div className="pb-2">
              <div
                ...
                <div className="flex items-center gap-2">
                  {gpsEnabled && gpsPermission === "granted" ? (
                    ...
                    <>
                      ...
                      <span className="text-xs text-[#2d332d]/60">
                        GPS Disabled - Simulated
                      </span>
                    </>
                  ) : (
                    <>
              ...
              <p className="mt-1 text-[10px] text-[#2d332d]/70">
                For accurate GPS tracking, keep this screen open. Locking your
                phone or leaving the app may pause location updates.
              </p>
            </div>
          )}
        </div>
              ...
              <Card className="bg-[#eef0ed] border-none p-6 rounded-2xl shadow-none">
                <div className="text-center">
                  <div className="flex items-center justify-center gap-2 mb-2">
                    ...
                    {gpsEnabled && (
                      <Navigation className="w-3 h-3 text-[#2d332d]" />
                    )}
                  </div>
                  <p className="text-5xl tracking-wider mb-1 text-[#2d332d]">
                    {formatDistance(distance)}
                ...
                <Card className="bg-[#eef0ed] border-none p-6 rounded-2xl shadow-none">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center border-r border-[#2d332d]/10">
                      <p className="text-[#2d332d]/60 text-xs mb-1">
                        Current Speed
                      </p>
                      <p className="text-3xl text-[#2d332d]">
            ...
            <Card className="bg-[#eef0ed] border-none p-4 rounded-2xl shadow-none">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[#2d332d] text-lg">
                    {Math.round(elapsedSeconds * 0.15)}
                  </p>
                </div>
              </div>
            </Card>
          ...
          <div className="max-w-2xl mx-auto space-y-3">
            ...
            {isLocked && (
              <div className="bg-[#2d332d]/20 border border-[#2d332d]/30 rounded-xl p-3 flex items-center justify-center gap-2 mb-2">
                ...
                <p className="text-sm text-[#2d332d]">
                  Workout is locked - Slide to unlock
                </p>
              </div>
            )}
            ...
            {isLocked ? (
              ...
              >
                ...
                <div className="absolute inset-0 flex items-center justify-end pr-32 gap-1 pointer-events-none opacity-30">
                  <ChevronRight
                    className="w-6 h-6 text-[#2d332d] animate-pulse"
                    style={{ animationDelay: "0ms" }}
                  />
                  <ChevronRight
                    className="w-6 h-6 text-[#2d332d] animate-pulse"
